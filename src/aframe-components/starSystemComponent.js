window.AFRAME.registerComponent('star-system', {
    schema: {
      color: {
        type: 'string',
        default: "#FFF"
    ...
    update: function() {
      ...
      if (this.data.texture) {
        ...
        texture.map = new window.THREE.TextureLoader().load(this.data.texture);
      }
  
      const stars = new window.THREE.BufferGeometry();
      console.log('stars geometry', stars)
      ...
      const starMaterial = new window.THREE.PointsMaterial(Object.assign(texture, {
        color: this.data.color,
        size: this.data.size
      }));
  
      // Add the star particles to the element
      this.el.setObject3D('star-system', new window.THREE.Points(stars, starMaterial));
    },
